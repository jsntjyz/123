...
import traceback
import numpy as np
#np.random.BitGenerator = np.random._bit_generator
import argparse
import json
import yaml
...
mpl.rcParams['font.family'] = 'simhei' #'STKAITI'——字体
...
class PathVisualizer:
    ...
    def __init__(self, scenario):
        ...
        self.map_name = "stage4"
        # self.map_name = "floor" # self.scenario_content["map"]
    ...
    #    # 获取地图图片大小
    #    img_width, img_height = Image.open(os.path.join(self.map_path, self.map_content["image"])).size
    ...
    #    # 设置extent参数
    #    ax.imshow(self.map_img, cmap="gray", extent=(x0, x1, y0, y1))

    #    return fig, ax

     

    def create_episode_plots_for_namespaces(self, dataset, title, save_name, episode=0, differentiate="local_planner", desired_results=[], should_add_obstacles=True, should_add_collisions=False):
    ...
    def add_obstacles_to_plot(self, ax, should_add_obstacles=True):
        if  should_add_obstacles:
            return
        
        # print(self.scenario_content)
        obstacles = self.scenario_content["pedsim_agents"]
        # print(obstacles)
        for o in obstacles:
            position = o["pos"]
            ...
            ax.plot(map_coordinates[0], map_coordinates[1], "o--", color="black", ms=10, alpha=0.2)
        ...
        for o in obstacles:
            ...
            ax.plot(x, y, "x--", lw=1, color="black", ms=10, alpha=0.1)

    def add_start_and_goal_to_plot(self, ax, start, goal):
        print("start")
        print(start[0],start[1])
        start = self.ros_to_real_coord(start)
        ax.plot(start[0], start[1], "x", label="Start", ms=20, color="green")
        print("goal")
        print(goal[0],goal[1])
        goal = self.ros_to_real_coord(goal)

        ax.plot(goal[0], goal[1], "*", label="Goal", ms=20, color="red")
